...
public class Balls_in_berland_c implements Runnable {
	...
	public static void main(String[] args) throws java.lang.Exception {
		new Thread(null, new Balls_in_berland_c(), "Main", 1 << 26).start();
	}

	void solve() {
		int a = fr.nextInt(),b=fr.nextInt(),k=fr.nextInt();
		int bo[] = fr.nextIntArray(k),gi[]=fr.nextIntArray(k);
		if(k<2) {
			out.print(0);
			return;
		}
		int pb[]=new int[k],pg[]=new int[k];
		int fb[]=new int[a+1],fg[]=new int[b+1];
		for(int i=k-1; i>=0; i--) {
			pb[i]=++fb[bo[i]]-1;
			pg[i]=++fg[gi[i]]-1;
		}
